...
sudo curl --silent --location -o /usr/local/bin/kubectl https://dl.k8s.io/release/v1.33.0/bin/linux/amd64/kubectl  > /dev/null
...
# Get current account ID and set WSParticipantRole
STS_INFO=$(aws sts get-caller-identity)
ACCOUNT_ID=$(echo $STS_INFO | jq -r '.Account')
export WS_ROLE_ARN="arn:aws:iam::${ACCOUNT_ID}:role/WSParticipantRole"


# Output : Prerequisites installed successfully.
